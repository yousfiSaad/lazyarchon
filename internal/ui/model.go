...
// NewModelWithPlugin creates a new application model using a plugin manager
func NewModelWithPlugin(cfg *configpkg.Config, manager *plugin.Manager) MainModel {
	...
	// Get the client from the plugin manager
	client, err := manager.GetActiveClient()
	if err != nil {
		logger.Error("Failed to get active client from plugin manager", "error", err)
		// Return a model with nil client - the UI will show connection error
	}
...
func createModelWithDependencies(
	...
	componentContext.GetSortedTasks = func() []interface{} {
		...
		// Convert []interfaces.Task to []interface{} to avoid import cycle
...
func (m MainModel) Init() tea.Cmd {
	cmds := []tea.Cmd{
		tasks.LoadTasksInterface(m.programContext.TaskClient, m.programContext.SelectedProjectID),
		projects.LoadProjectsInterface(m.programContext.TaskClient),
...
func (m *MainModel) updateTasks(tasks []interfaces.Task) {
	startTime := time.Now()
...
func (m *MainModel) adjustTaskSelection(sortedTasks []interfaces.Task) {
...
func (m *MainModel) updateTaskListComponents(sortedTasks []interfaces.Task) tea.Cmd {
...
func (m *MainModel) updateProjects(projects []interfaces.Project) {
...
func (m *MainModel) updateProjectListComponent(projects []interfaces.Project) tea.Cmd {
...
func (m *MainModel) validateTaskSelection() (*interfaces.Task, bool) {
	sortedTasks := m.GetSortedTasks()
	if len(sortedTasks) == 0 || m.uiState.SelectedTaskIndex >= len(sortedTasks) {
		return nil, false
	}
...
func (m MainModel) GetTasks() []interfaces.Task {
	return m.programContext.Tasks
}

// GetProjects returns the current projects
func (m MainModel) GetProjects() []interfaces.Project {
...
func (m MainModel) GetSortedTasks() []interfaces.Task {
...
func (m MainModel) GetSelectedTask() *interfaces.Task {
	if m.components.Layout.MainContent == nil {
		return nil
	}
...
func (m MainModel) GetSelectedProject() *interfaces.Project {
	if m.programContext.SelectedProjectID == nil {
		return nil
	}
