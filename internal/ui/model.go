...
import (
	...
	"github.com/yousfisaad/lazyarchon/internal/logging"
	configpkg "github.com/yousfisaad/lazyarchon/internal/shared/config"
	...
	"github.com/yousfisaad/lazyarchon/internal/shared/utils"
	...
	"github.com/yousfisaad/lazyarchon/internal/ui/components/modals/statusfilter"
	...
	"github.com/yousfisaad/lazyarchon/internal/ui/components/taskdetails"
	"github.com/yousfisaad/lazyarchon/internal/ui/components/tasklist"
	"github.com/yousfisaad/lazyarchon/internal/ui/context"
	"github.com/yousfisaad/lazyarchon/internal/ui/factories"
	"github.com/yousfisaad/lazyarchon/internal/ui/helpers"
	"github.com/yousfisaad/lazyarchon/internal/ui/messages"
	"github.com/yousfisaad/lazyarchon/internal/ui/sorting"
	stylingprovider "github.com/yousfisaad/lazyarchon/internal/ui/styling"
)
...
type MainModel struct {
	...
	wsClient       interfaces.RealtimeClient  // WebSocket connection for realtime updates
...
func NewModel(cfg *configpkg.Config) MainModel {
	...
	wsClient.SetLogger(logger) // Inject logger for WebSocket event logging

	// Delegate to shared model creation logic
	return createModelWithDependencies(client, wsClient, cfg, styleContextProvider, logger)
}
...
func createModelWithDependencies(
	...
	model := buildModel(wsClient, programContext, uiState, components, config)
...
func (m MainModel) Init() tea.Cmd {
	...
	// Only initialize WebSocket if realtime is enabled
	// NOTE: Backend doesn't support WebSocket - use HTTP polling instead
	if m.isRealtimeEnabled() {
		cmds = append(cmds,
			realtime.InitializeRealtimeCmd(m.wsClient),   // Initialize WebSocket connection
			realtime.ListenForRealtimeEvents(m.wsClient), // Start listening for events
		)
	} else {
		// Use HTTP polling for auto-refresh when WebSocket is disabled
...
func (m *MainModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case realtime.RealtimeConnectedMsg, realtime.RealtimeDisconnectedMsg,
		realtime.RealtimeTaskUpdateMsg, realtime.RealtimeTaskCreateMsg,
		realtime.RealtimeTaskDeleteMsg, realtime.RealtimeProjectUpdateMsg:
		return m.handleRealtimeMessages(msg)
