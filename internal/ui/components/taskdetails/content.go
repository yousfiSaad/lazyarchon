...
func (c *TaskContentGenerator) generateTaskMetadata(task *interfaces.Task, factory *styling.StyleFactory) []string {
	...
	if c.context != nil && c.context.ConfigProvider != nil && c.context.ConfigProvider.IsPriorityIndicatorsEnabled() {
		priority := styling.GetTaskPriority(task.Priority, nil)
		...
		orderText := factory.Text(styling.CurrentTheme.MutedColor).Render(fmt.Sprintf("(order: %d)", task.Priority))
		...
		taskOrderValue := factory.Text(styling.CurrentTheme.MutedColor).Render(fmt.Sprintf("%d", task.Priority))
