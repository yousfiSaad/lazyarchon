...
func generateTestTasks(count int) []interfaces.Task {
	tasks := make([]interfaces.Task, count)
	...
	for i := 0; i < count; i++ { //nolint:varnamelen // i is idiomatic for loop index
		tasks[i] = interfaces.Task{
			...
			Tags:        []string{features[i%len(features)]},
			Priority:    i % 10,
...
func createBenchmarkModel(tasks []interfaces.Task, width, height int) TaskListModel {
