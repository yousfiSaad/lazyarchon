...
func (m *TaskListModel) getSortedTasks() []interfaces.Task {
	...
	// Convert []interface{} back to []interfaces.Task
	tasks := make([]interfaces.Task, len(interfaceTasks))
	for i, t := range interfaceTasks {
		if task, ok := t.(interfaces.Task); ok {
			tasks[i] = task
		}
	}
...
func (m *TaskListModel) GetSelectedTask() *interfaces.Task {
...
func (m *TaskListModel) matchesSearch(task interfaces.Task) bool {
	if m.searchQuery == "" {
		return false
	}
