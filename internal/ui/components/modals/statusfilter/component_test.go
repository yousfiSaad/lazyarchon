...
func TestStatusFilterModalShow(t *testing.T) {
	...
	currentStatuses := map[string]bool{
		interfaces.StatusTodo:  true,
		interfaces.StatusDoing: false,
		interfaces.StatusDone:  true,
	}
	...
	if !modal.selectedStatuses[interfaces.StatusTodo] {
		t.Error("Expected todo status to be selected")
	}
	if modal.selectedStatuses[interfaces.StatusDoing] {
		t.Error("Expected doing status to be unselected")
	}
	if !modal.selectedStatuses[interfaces.StatusDone] {
		t.Error("Expected done status to be selected")
	}

	// Verify backup was created
	if !modal.backupStatuses[interfaces.StatusTodo] {
...
func TestStatusFilterModalToggleStatus(t *testing.T) {
	...
	modal.filteredStatuses = []string{interfaces.StatusTodo, interfaces.StatusDoing}
...
func TestStatusFilterModalNavigation(t *testing.T) {
	modal := createTestStatusFilterModal()
	modal.SetActive(true)
	modal.filteredStatuses = []string{interfaces.StatusTodo, interfaces.StatusDoing, interfaces.StatusDone}
...
func TestStatusFilterModalSearch(t *testing.T) {
	...
	expectedFiltered := []string{interfaces.StatusTodo}
...
func TestStatusFilterModalCancel(t *testing.T) {
	...
	modal.selectedStatuses[interfaces.StatusTodo] = false
	modal.selectedStatuses[interfaces.StatusDoing] = true
	...
	if modal.selectedStatuses[interfaces.StatusTodo] != originalStatuses[interfaces.StatusTodo] {
		t.Error("Expected todo status to be restored from backup")
	}
	if modal.selectedStatuses[interfaces.StatusDoing] != originalStatuses[interfaces.StatusDoing] {
...
func TestStatusFilterModalApply(t *testing.T) {
	...
	if batchCmd, ok := msg.(tea.BatchMsg); ok {
		...
		if appliedMsg == nil {
			...
			if !appliedMsg.SelectedStatuses[interfaces.StatusTodo] {
				t.Error("Expected todo status to be selected in applied message")
			}
			if appliedMsg.SelectedStatuses[interfaces.StatusDoing] {
				t.Error("Expected doing status to be unselected in applied message")
			}
			if !appliedMsg.SelectedStatuses[interfaces.StatusDone] {
