...
// High complexity (17) due to comprehensive key routing for project navigation
//
...
func (m *MainModel) handleQuitKey(key string) (tea.Cmd, bool) {
	...
	if m.HasActiveModal() {
		...
		case m.components.Modals.StatusModel.IsActive():
			return func() tea.Msg { return status.HideStatusModalMsg{} }, true
		case m.uiState.IsProjectView():
		...
		case m.components.Modals.TaskEditModel.IsActive():
...
func (m *MainModel) handleRefreshKey(key string) (tea.Cmd, bool) {
	...
	cmds = append(cmds, projects.RefreshDataInterface(m.programContext.TaskClient, m.programContext.SelectedProjectID))
