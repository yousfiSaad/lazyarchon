...
// CreateTask creates a new task with resilience
func (rc *ResilientClient) CreateTask(request CreateTaskRequest) (*TaskResponse, error) {
	var result *TaskResponse
	var lastErr error
	...
	operation := func() error {
		var err error
		result, err = rc.client.CreateTask(request)
		lastErr = err
		return err
	}
...
// NewResilientClientWithDefaults creates a resilient client with default configuration and 90 second timeout
func NewResilientClientWithDefaults(baseURL, apiKey string, timeout time.Duration) *ResilientClient {
	return NewResilientClient(baseURL, apiKey, timeout, DefaultResilienceConfig())
}
...
func NewClientFactory(baseURL, apiKey string, timeout time.Duration) *ClientFactory {
...
func (cf *ClientFactory) CreateClient() ClientInterface {
	...
	return NewClient(cf.baseURL, cf.apiKey, cf.timeout)
}
