...
// SetDefault makes this logger the global slog default, so bare slog
// calls write to the log file instead of stderr — required for the stdio
// MCP server, where stderr noise confuses clients and stdout is the protocol.
func (l *SlogLogger) SetDefault() {
...
func (l *SlogLogger) LogPerformance(operation string, startTime time.Time, args ...interface{}) {
	...
	l.logger.LogAttrs(context.Background(), slog.LevelDebug, "Performance", attrs...)
}
...
func (l *SlogLogger) LogPerformanceWithMemory(operation string, startTime time.Time, args ...interface{}) {
	...
	attrs = append(attrs,
		...
		slog.Uint64("sys_mb", memStats.Sys/1024/1024),
		slog.Uint64("num_gc", uint64(memStats.NumGC)),
	)

	l.logger.LogAttrs(context.Background(), slog.LevelDebug, "Performance (with memory)", attrs...)
}
...
func (l *SlogLogger) LogHTTPRequest(method, url string, args ...interface{}) {
	...
	l.logger.LogAttrs(context.Background(), slog.LevelDebug, "HTTP Request", attrs...)
}
...
func (l *SlogLogger) LogHTTPResponse(method, url string, statusCode int, duration time.Duration, args ...interface{}) {
	...
	l.logger.LogAttrs(context.Background(), level, "HTTP Response", attrs...)
}
...
func (l *SlogLogger) LogStateChange(component, field string, oldValue, newValue interface{}, args ...interface{}) {
	...
	l.logger.LogAttrs(context.Background(), slog.LevelDebug, "State Change", attrs...)
}
...
func (l *SlogLogger) LogComponentUpdate(component, action string, args ...interface{}) {
	...
	l.logger.LogAttrs(context.Background(), slog.LevelDebug, "Component Update", attrs...)
}
