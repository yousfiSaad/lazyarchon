...
const (
	defaultServerURL   = "http://localhost:8181"
	defaultProfileName = "development"
)
...
type Config struct {
	...
	Plugin      string            `yaml:"plugin" validate:"omitempty,min=1"` // Plugin name to use (e.g., "archon", "github")
	Server      ServerConfig      `yaml:"server" validate:"required"`
	UI          UIConfig          `yaml:"ui" validate:"required"`
	Development DevelopmentConfig `yaml:"development" validate:"required"`
	Plugins     PluginsConfig     `yaml:"plugins,omitempty"` // Per-plugin configurations
}

// PluginsConfig holds per-plugin configuration
type PluginsConfig struct {
	Local   LocalConfig  `yaml:"local,omitempty"`   // Local SQLite plugin configuration
	Archon  PluginConfig `yaml:"archon,omitempty"`  // Archon plugin configuration
	Gitea   PluginConfig `yaml:"gitea,omitempty"`   // Gitea plugin configuration
	Vikunja PluginConfig `yaml:"vikunja,omitempty"` // Vikunja plugin configuration
}

// LocalConfig holds configuration for the local SQLite plugin
type LocalConfig struct {
	// Path is the database file location. Empty = the plugin's default
	// data directory (see local.DefaultDataDir).
	Path string `yaml:"path,omitempty"`
}

// PluginConfig holds configuration for a specific plugin
type PluginConfig struct {
	BaseURL       string            `yaml:"base_url,omitempty"`       // API base URL (overrides global server URL)
	APIKey        string            `yaml:"api_key,omitempty"`        // API key (overrides global API key)
	AuthToken     string            `yaml:"auth_token,omitempty"`     // OAuth/token auth
	Username      string            `yaml:"username,omitempty"`       // Basic auth username
	Password      string            `yaml:"password,omitempty"`       // Basic auth password
	Timeout       time.Duration     `yaml:"timeout,omitempty"`        // Request timeout (0 = use global)
	CustomHeaders map[string]string `yaml:"custom_headers,omitempty"` // Additional headers
	Mappings      FieldMappings     `yaml:"mappings,omitempty"`       // Field/status mappings
}

// FieldMappings defines how generic fields map to backend-specific fields
type FieldMappings struct {
	Status    map[string]string      `yaml:"status,omitempty"`     // Status mappings (e.g., {"todo": "open"})
	Priority  map[string]interface{} `yaml:"priority,omitempty"`   // Priority mappings
	FieldName map[string]string      `yaml:"field_name,omitempty"` // Field name mappings
	Tags      map[string]string      `yaml:"tags,omitempty"`       // Tag mappings
}

// ServerConfig holds server-related configuration
type ServerConfig struct {
	// URL of the HTTP backend. Only required for HTTP plugins (archon,
	// gitea, vikunja); the local plugin ignores it.
...
var defaultConfig = Config{
	...
	Plugin:  "local", // Standalone by default; set "archon"/"gitea"/"vikunja" for an HTTP backend
	Server: ServerConfig{
...
func (c *Config) applyEnvironmentOverrides() {
	...
	if dbPath := os.Getenv("LAZYARCHON_DB_PATH"); dbPath != "" {
		c.Plugins.Local.Path = dbPath
	}
}

// LocalDBPath returns the local SQLite database location: the
// LAZYARCHON_DB_PATH environment variable, then plugins.local.path, then
// empty (meaning the local plugin picks its own default data directory).
func (c *Config) LocalDBPath() string {
	return c.Plugins.Local.Path
}
...
func (c *Config) applyProfileDefaults() {
	...
	if c.Profile == "" {
		if profile := os.Getenv("LAZYARCHON_PROFILE"); profile != "" {
			...
			c.Profile = defaultProfileName // Default profile
		}
	}
	...
	case "development", "dev":
		...
		if c.Server.URL == defaultServerURL { // Only override if using default
...
// GetPlugin returns the configured plugin name (defaults to "local")
func (c *Config) GetPlugin() string {
	if c.Plugin == "" {
		return "local"
	}
	return c.Plugin
...
// The local plugin has no HTTP settings; its database path travels via
// PluginConfig.Extra["path"] (see bootstrap.InitPlugin and LocalDBPath).
func (c *Config) GetPluginConfig(pluginName string) PluginConfig {
	switch pluginName {
	case "archon":
		return c.Plugins.Archon
	case "gitea":
		return c.Plugins.Gitea
	case "vikunja":
		return c.Plugins.Vikunja
	case "local":
		return PluginConfig{}
	default:
		return PluginConfig{}
...
// BuildPluginConfig builds a complete plugin.PluginConfig from settings
func (c *Config) BuildPluginConfig(pluginName string) interface{} {
	// This will be used by the plugin manager
	// Returns a map that can be converted to plugin.PluginConfig
	pluginConfig := c.GetPluginConfig(pluginName)
	...
	return map[string]interface{}{
		"base_url":       baseURL,
		"api_key":        apiKey,
		"auth_token":     pluginConfig.AuthToken,
		"username":       pluginConfig.Username,
		"password":       pluginConfig.Password,
		"timeout":        timeout,
		"custom_headers": pluginConfig.CustomHeaders,
		"mappings":       pluginConfig.Mappings,
	}
}
