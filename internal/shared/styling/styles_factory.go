...
func (f *StyleFactory) Priority(priority string) lipgloss.Style {
	var color string
	switch priority {
	case PriorityHighStr:
		color = "#FF6B6B" // Red for high priority
	case PriorityMediumStr:
...
const (
	// borderOverhead is the space consumed by RoundedBorder.
	// Vertical: Top border (1) + Bottom border (1) = 2 lines
	// Horizontal: Left border (1) + Right border (1) = 2 chars
	borderOverhead = 2
)

// Panel creates a bordered panel style that renders within the specified dimensions.
// Both width and height parameters represent TOTAL rendered output.
// Borders are rendered INSIDE the allocated space, not added to it.
func (f *StyleFactory) Panel(width, height int, isActive bool) lipgloss.Style {
	...
	// Subtract border overhead so total output (content + borders) = allocated dimensions
	contentHeight := height - borderOverhead
	contentWidth := width - borderOverhead

	return lipgloss.NewStyle().
		Border(lipgloss.RoundedBorder()).
		BorderForeground(lipgloss.Color(borderColor)).
		Width(contentWidth).   // Width for content + padding (borders outside this)
		Height(contentHeight). // Height for content (borders outside this)
		Padding(0, 0, 0, 0)    // No padding, content flush to borders
}
...
func (f *StyleFactory) StatusBar(state string) lipgloss.Style {
	...
	switch state {
	case "ready":
		return baseStyle.Background(lipgloss.Color("68")) // ANSI Blue for ready/connected
	case "loading":
		return baseStyle.Background(lipgloss.Color("214")) // ANSI Orange for active/working
	case "error":
		return baseStyle.Background(lipgloss.Color("160")) // ANSI Red for error
	case "info":
		return baseStyle.Background(lipgloss.Color("208")) // ANSI Dark Orange for info/feedback
	default:
		return baseStyle.Background(lipgloss.Color("240")) // ANSI Gray
	}
}
