...
func (b *TaskLineBuilder) AddPriorityIndicator(task plugin.Task) *TaskLineBuilder {
	if !b.styleContext.StyleProvider().IsPriorityIndicatorsEnabled() {
		return b
	}

	priority := GetTaskPriority(task.Priority, nil)
...
func (b *TaskLineBuilder) AddStatusIndicator(task plugin.Task) *TaskLineBuilder {
	symbol := plugin.GetStatusSymbol(task.Status)
...
func (b *TaskLineBuilder) AddTitle(task plugin.Task, searchQuery string, searchActive bool) *TaskLineBuilder {
	var content string
...
// AddFeatureTag adds the first tag if present and space permits
func (b *TaskLineBuilder) AddFeatureTag(task plugin.Task) *TaskLineBuilder {
	if len(task.Tags) == 0 || task.Tags[0] == "" {
		return b
	}

	content := fmt.Sprintf(" #%s", task.Tags[0])
	var style lipgloss.Style

	if b.styleContext.StyleProvider().IsFeatureColorsEnabled() {
		style = b.styleContext.Factory().Feature(task.Tags[0])
	} else {
