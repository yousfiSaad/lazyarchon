...
func GetTaskPriority(taskOrder int, allTasks []plugin.Task) PriorityLevel {
	// Generic priority scale: 1 (critical) to 4 (low).
	if taskOrder >= plugin.PriorityCritical && taskOrder <= plugin.PriorityLow {
		switch taskOrder {
		case plugin.PriorityCritical, plugin.PriorityHigh:
			return PriorityHigh
	...
	// Legacy Archon task_order scale (0-100, higher = more urgent).
	if taskOrder >= 80 {
		return PriorityHigh
