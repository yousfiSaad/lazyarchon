...
func (s *store) migrate() error {
	...
	for v := version; v < len(migrations); v++ {
		tx, err := s.db.Begin()
		if err != nil {
			return fmt.Errorf("failed to start migration %d: %w", v+1, err)
		}
...
func scanTask(row interface{ Scan(...any) error }) (plugin.Task, error) {
	var r taskRow
	if err := row.Scan(
		&r.ID, &r.ProjectID, &r.ParentID, &r.Title, &r.Description, &r.Status,
		&r.Priority, &r.Assignee, &r.Tags, &r.DueDate, &r.Sources, &r.CodeExamples,
		&r.Extra, &r.Archived, &r.ArchivedAt, &r.CreatedAt, &r.UpdatedAt,
	); err != nil {
