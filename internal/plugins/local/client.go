...
func (c *LocalClient) UpdateTask(ctx context.Context, taskID string, request plugin.UpdateTaskRequest) (*plugin.Task, error) {
	...
	if request.Status != nil && !isValidStatus(*request.Status) {
		return nil, validationError("UpdateTask", fmt.Sprintf("invalid status %q (want todo, doing, review or done)", *request.Status))
	}

	if request.Priority != nil && !isValidPriority(*request.Priority) {
		return nil, validationError("UpdateTask", fmt.Sprintf("invalid priority %d (want 1-4, lower is more urgent)", *request.Priority))
	}

	if request.Title != nil && strings.TrimSpace(*request.Title) == "" {
		return nil, validationError("UpdateTask", "title cannot be empty")
	}
	...
	if request.ProjectID != nil {
		...
		if !projectExists {
			return nil, notFound("UpdateTask", "project "+*request.ProjectID)
		}
	}

	if request.ParentID != nil && *request.ParentID != "" {
		if *request.ParentID == taskID {
			return nil, validationError("UpdateTask", "a task cannot be its own parent")
		}
		...
		if !parentExists {
			return nil, notFound("UpdateTask", "parent task "+*request.ParentID)
		}
		...
		if cycle {
			return nil, validationError("UpdateTask", "setting this parent would create a task cycle")
		}
	}
