...
func (p *ArchonPlugin) CreateClient(config plugin.PluginConfig) (plugin.TaskClient, error) {
	...
	if len(config.CustomHeaders) > 0 {
		// The base client doesn't support custom headers directly,
		// but we can store them in the adapter for potential future use
	}
...
func (a *ArchonClientAdapter) CreateTask(ctx context.Context, request plugin.CreateTaskRequest) (*plugin.Task, error) {
	...
	if request.ParentID != nil {
		// Archon uses a different field name for parent tasks
		// We'll store it in the Extra field and handle it in the request
		// Note: This would need to be handled by the actual Archon API
	}
...
func (a *ArchonClientAdapter) convertTask(t Task) plugin.Task {
	// Extract tags from the feature field
	tags := []string{}
	if t.Feature != nil && *t.Feature != "" {
		tags = append(tags, *t.Feature)
	}
	...
	if t.ArchivedAt != nil {
		archivedTime := t.ArchivedAt.Time
	...
	return plugin.Task{
		...
		Status:      a.fromBackendStatus(t.Status),
		Priority:    t.TaskOrder,
		Assignee:    t.Assignee,
		Tags:        tags,
		CreatedAt:   createdAt,
		UpdatedAt:   updatedAt,
		...
		Extra: map[string]interface{}{
			"sources":       t.Sources,
			"code_examples": t.CodeExamples,
		},
	}
}

// convertProject converts an Archon Project to a generic plugin.Project
func (a *ArchonClientAdapter) convertProject(p Project) plugin.Project {
...
// init registers the Archon plugin with the global registry
func init() {
	plugin.Register(&ArchonPlugin{})
}
