...
func TestResilientClient_Integration(t *testing.T) {
	...
	client := NewResilientClient(server.URL, "test-key", 30*time.Second, config)
...
func TestClientFactory(t *testing.T) {
	factory := NewClientFactory("http://localhost:8181", "test-key", 30*time.Second)
...
func BenchmarkResilientClient_Success(b *testing.B) {
	...
	client := NewResilientClientWithDefaults(server.URL, "test-key", 30*time.Second)
