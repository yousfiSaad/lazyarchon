...
// TestProjectWriteOperationsUnsupported verifies the Archon adapter reports
...
func TestProjectWriteOperationsUnsupported(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "UpdateProject",
			call: func() error {
				_, err := adapter.UpdateProject(ctx, "proj-1", plugin.UpdateProjectRequest{})
				return err
			},
		},
		{
			name: "DeleteProject",
			call: func() error {
				return adapter.DeleteProject(ctx, "proj-1")
			},
		},
	}
